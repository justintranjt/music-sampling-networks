...
with open('whosampled5k.csv', mode='r', encoding="cp850") as dataSamples:
...
# (largest_hub, degree) = sorted(most_samples.items(), key=itemgetter(1))[-2]
largest_hub = 'Girl Talk'
...
hub_ego = networkx.ego_graph(diG, largest_hub, distance=20)
# Construct ego graph links
for link in links:
	if largest_hub in link:
		hub_ego.add_node(link[0], genre=link[3])
		hub_ego.add_edge(u_of_edge=link[0], v_of_edge=largest_hub, audioElem=link[2])
...
for artist in original_hub_nodes:
	...
	for link in links:
		if artist in link and link[0] not in original_hub_nodes:
			hub_ego.add_node(link[0], genre=link[3])
			hub_ego.add_edge(u_of_edge=link[0], v_of_edge=artist)
...
networkx.draw_networkx_edge_labels(hub_ego, pos, edge_labels, font_size=4)
...
# Draw graph
networkx.draw(hub_ego, pos, cmap=plt.cm.RdYlBu, node_color=color_map_genre, node_size=50, with_labels=True, font_size=10)
del color_map_genre[0]

# Draw ego graph based on genre color
networkx.draw_networkx_nodes(hub_ego, pos, nodelist=largest_hub_samplers, node_size=50,
	cmap=plt.cm.RdYlBu)
plt.title('Ego Graph for 1st Most-Sampling Artist in Network (Girl Talk)', size=12)
# plt.savefig('egoGraphMostSampling1GirlTalk.png', dpi=199)
plt.show()
